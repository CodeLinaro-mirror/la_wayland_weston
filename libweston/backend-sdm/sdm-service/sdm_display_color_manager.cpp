...
DisplayError SDMQDCMModeManager::EnableActiveFeatures(bool enable,
...
DisplayError SDMQDCMModeManager::EnableQDCMMode(bool enable,
                                                SdmDisplayProxy *sdmdisp) {
  DisplayError ret = kErrorNone;

  ret = EnableActiveFeatures((enable ? false : true), kActiveFeatureCMD[kCABLFeature],
                             &cabl_was_running_);
  if (ret != kErrorNone) {
    DLOGE("Failed to enable active features, error = %d", ret);
    return ret;
  }
...
DisplayError SDMColorManager::CreatePayloadFromParcel(const android::Parcel &in,
                                             uint32_t *disp_id,
...
DisplayError SDMColorManager::EnableQDCMMode(bool enable, SdmDisplayProxy *sdmdisplay) {
  DisplayError ret = kErrorNone;
...
DisplayError SDMColorManager::SetHWDetailedEnhancerConfig(void *params,
                              SdmDisplayProxy *sdmdisp) {
...
DisplayError SDMColorManager::SetDetailedEnhancer(void *params, SdmDisplayProxy *sdmdisp) {
  SCOPE_LOCK(locker_);
  DisplayError err = kErrorNone;
